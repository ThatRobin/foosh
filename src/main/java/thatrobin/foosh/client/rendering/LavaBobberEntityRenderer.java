...
public class LavaBobberEntityRenderer extends EntityRenderer<LavaBobberEntity> {
    private static final Identifier TEXTURE = new Identifier("foosh", "textures/entity/blaze_hook.png");
    private static final RenderLayer LAYER;

    public LavaBobberEntityRenderer(EntityRendererFactory.Context context) {
        super(context);
    }

    public void render(LavaBobberEntity fishingBobberEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        ...
        if (playerEntity != null) {
            ...
            if (!itemStack.isOf(FooshItems.BLAZE_ROD)) {
                j = -j;
            }
            ...
            if ((this.dispatcher.gameOptions == null || this.dispatcher.gameOptions.getPerspective().isFirstPerson()) && playerEntity == MinecraftClient.getInstance().player) {
                s = 960.0D / (double)(Integer)this.dispatcher.gameOptions.getFov().getValue();
                ...
                o = MathHelper.lerp((double)g, playerEntity.prevX, playerEntity.getX()) - e * m - d * 0.8D;
                p = playerEntity.prevY + (double)playerEntity.getStandingEyeHeight() + (playerEntity.getY() - playerEntity.prevY) * (double)g - 0.45D;
                q = MathHelper.lerp((double)g, playerEntity.prevZ, playerEntity.getZ()) - d * m + e * 0.8D;
    ...
    public Identifier getTexture(LavaBobberEntity fishingBobberEntity) {
        return TEXTURE;
    }
