...
public class LavaBobberEntity extends FishingBobberEntity {
    ...
    public LavaBobberEntity.State state;
    public final int luckOfTheSeaLevel;
    public final int lureLevel;

    private LavaBobberEntity(EntityType<? extends LavaBobberEntity> type, World world, int luckOfTheSeaLevel, int lureLevel) {
        super(type, world);
    ...
    public LavaBobberEntity(EntityType<? extends LavaBobberEntity> entityType, World world) {
        this(entityType, world, 0, 0);
    }

    @SuppressWarnings("all")
    public LavaBobberEntity(PlayerEntity thrower, World world, int luckOfTheSeaLevel, int lureLevel) {
        this(FooshEntities.LAVA_BOBBER, world, luckOfTheSeaLevel, lureLevel);
    ...
    public void tick() {
        ...
        if (playerEntity == null) {
            ...
            if (fluidState.isIn(FluidTags.LAVA)  || fluidState.isIn(FluidTags.WATER)) {
                f = fluidState.getHeight(this.world, blockPos);
            }

            boolean bl = f > 0.0F;
            if (this.state == LavaBobberEntity.State.FLYING) {
                if (this.hookedEntity != null) {
                    this.setVelocity(Vec3d.ZERO);
                    this.state = LavaBobberEntity.State.HOOKED_IN_ENTITY;
                    return;
                }

                if (bl) {
                    this.setVelocity(this.getVelocity().multiply(0.3D, 0.2D, 0.3D));
                    this.state = LavaBobberEntity.State.BOBBING;
                    return;
                }

                this.checkForCollision();
            } else {
                if (this.state == LavaBobberEntity.State.HOOKED_IN_ENTITY) {
                    if (this.hookedEntity != null) {
                        if (!this.hookedEntity.isRemoved() && this.hookedEntity.world.getRegistryKey() == this.world.getRegistryKey()) {
                            ...
                            this.state = LavaBobberEntity.State.FLYING;
                        }
                    }
                ...
                if (this.state == LavaBobberEntity.State.BOBBING) {
            ...
            this.updateRotation();
            if (this.state == LavaBobberEntity.State.FLYING && (this.onGround || this.horizontalCollision)) {
                this.setVelocity(Vec3d.ZERO);
            }
            ...
            if (this.fishTravelCountdown > 0) {
                ...
                if (this.fishTravelCountdown > 0) {
                    ...
                    if(blockState.isOf(Blocks.LAVA)) {
                        if (this.random.nextFloat() < 0.15F) {
                            serverWorld.spawnParticles(ParticleTypes.LAVA, d, e - 0.10000000149011612D, j, 1, g, 0.1D, h, 0.0D);
                        }

                        float k = g * 0.04F;
                        float l = h * 0.04F;
                        serverWorld.spawnParticles(ParticleTypes.SMOKE, d, e, j, 0, l, 0.01D, -k, 1.0D);
                        serverWorld.spawnParticles(ParticleTypes.SMOKE, d, e, j, 0, -l, 0.01D, k, 1.0D);
                    }
                } else {
                    ...
                    if(blockState.isOf(Blocks.LAVA)) {
                        serverWorld.spawnParticles(ParticleTypes.SMOKE, this.getX(), m, this.getZ(), (int) (1.0F + this.getWidth() * 20.0F), this.getWidth(), 0.0D, this.getWidth(), 0.20000000298023224D);
                        serverWorld.spawnParticles(ParticleTypes.LAVA, this.getX(), m, this.getZ(), (int) (1.0F + this.getWidth() * 20.0F),this.getWidth(), 0.0D, this.getWidth(), 0.20000000298023224D);
                    }
                ...
                if (this.random.nextFloat() < f) {
                    ...
                    if (blockState.isOf(Blocks.LAVA)) {
                        serverWorld.spawnParticles(ParticleTypes.SMOKE, d, e, j, 2 + this.random.nextInt(2), 0.10000000149011612D, 0.0D, 0.10000000149011612D, 0.0D);
                    }
                }
    ...
    private boolean isOpenOrWaterAround(BlockPos pos) {
        LavaBobberEntity.PositionType positionType = LavaBobberEntity.PositionType.INVALID;

        for(int i = -1; i <= 2; ++i) {
            LavaBobberEntity.PositionType positionType2 = this.getPositionType(pos.add(-2, i, -2), pos.add(2, i, 2));
            switch(positionType2) {
                case INVALID:
                    return false;
                case ABOVE_WATER:
                    if (positionType == LavaBobberEntity.PositionType.INVALID) {
                        return false;
                    }
                    break;
                case INSIDE_WATER:
                    if (positionType == LavaBobberEntity.PositionType.ABOVE_WATER) {
                        return false;
                    }
            }
    ...
    private LavaBobberEntity.PositionType getPositionType(BlockPos start, BlockPos end) {
        return BlockPos.stream(start, end).map(this::getPositionType).reduce((positionType, positionType2) ->
            positionType == positionType2 ? positionType : LavaBobberEntity.PositionType.INVALID
        ).orElse(LavaBobberEntity.PositionType.INVALID);
    }

    private LavaBobberEntity.PositionType getPositionType(BlockPos pos) {
        BlockState blockState = this.world.getBlockState(pos);

        if (!blockState.isAir() && !blockState.isOf(Blocks.LILY_PAD)) {
            FluidState fluidState = blockState.getFluidState();
            return fluidState.isIn(FluidTags.LAVA) && fluidState.isStill() && blockState.getCollisionShape(this.world, pos).isEmpty() ? LavaBobberEntity.PositionType.INSIDE_WATER : LavaBobberEntity.PositionType.INVALID;
        } else {
            return LavaBobberEntity.PositionType.ABOVE_WATER;
        }
    }
            ...
            if (this.hookedEntity != null) {
                ...
                if (blockState.isOf(Blocks.WATER)) {
                    ...
                    if (world.getRegistryKey() == World.NETHER) {
                        table = Objects.requireNonNull(this.world.getServer()).getLootManager().getTable(FooshLootTables.LAVA_FISHING_NETHER);
                    } else {
                        table = Objects.requireNonNull(this.world.getServer()).getLootManager().getTable(FooshLootTables.LAVA_FISHING_OVERWORLD);
                    }
                }
